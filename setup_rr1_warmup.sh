...
#python -m torch.distributed.launch --nproc_per_node=8 main.py --suffix=Net"$1"Bz"$3"by8Lr"$2"SbnStp35-50-60 --node_rank=0 --arch="$1" --syncbn --lr="$2" --batch-size="$3" --dropout=0.5 --epochs=65 --lr_steps 35 50 60 --dense_sample --num_dense_sample=32 --dense_sample_stride=1 --dataset="$4" --training_size="$5" --root_log=/msraimscratch/v-yizzh/log/log --root_model=/msraimscratch/v-yizzh/log/checkpoint --root_path=/msraimscratch/v-yizzh/data --print-freq=50 --eval-freq=2 --workers=8 --warmup --philly-nccl-multi-node --break_resume

if [ "$6" == "True" ]
then
    python -m torch.distributed.launch --nproc_per_node=8 --nnodes=1 --node_rank=$NODE_RANK --master_addr="$MASTER_IP" --master_port=$MASTER_PORT main.py --suffix=Net"$1"Bz"$3"by8Lr"$2"SbnStp30-40 --node_rank=$NODE_RANK --net_version="$1" --syncbn --lr="$2" --batch-size="$3" --dropout=0.5 --epochs=50 --lr_steps 30 40 --dense_sample --num_dense_sample=32 --dense_sample_stride=2 --random_dense_sample_stride --dataset="$4" --training_size="$5" --root_log=/log/log --root_model=/log/checkpoint --root_path=/data --print-freq=50 --eval-freq=2 --workers=8 --warmup --philly-nccl-multi-node --break_resume --use_zip
else
