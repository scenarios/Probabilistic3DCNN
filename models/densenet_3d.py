...
#Conv2d = Conv2d
Conv3d = partial(Conv3d_with_CD, weight_reg=args.weight_reg, deterministic=True if args.selection_mode or args.finetune_mode else False, training_size=args.training_size, p_init=args.p_init)
Linear = partial(Linear_with_CD, weight_reg=args.weight_reg, deterministic=True if args.selection_mode or args.finetune_mode else False, training_size=args.training_size, p_init=args.p_init)
nnConv2d = nn.Conv2d
BatchNorm3d = partial(nn.BatchNorm3d, track_running_stats=not args.freeze_bn)
_TEMPORAL_NASAS_ONLY = args.temporal_nasas_only
_TEMPORAL_NODOWNSAMPLE = 'v1d3' in args.net_version or ('pure' in args.net_version and ('something' in args.dataset or 'ucf' in args.dataset))
        ...
        if res:
            ...
            if 'conv' in key:
                ...
                if 'conv0' in key:
                    ...
                    if 'v1' in args.net_version or 'pure_temporal' in args.net_version:
                    ...
                    if 'transition' in key:
                        ...
                        state_dict[new_key_btnk] = v
                        if args.net_version in ['v1', 'v1d2', 'vt', 'v1d3', 'pure_temporal']:
                            new_key_tmpr = res.group(1) + 'temporal.' + res.group(2)
                            state_dict[new_key_tmpr] = v.repeat([1, 1, 3, 1, 1]) / 3.0
                        del state_dict[key]
                    else:
                ...
                if 'norm1' in key:
                    new_key_btnk = res.group(1) + 'bottleneck.' + res.group(2)
                    state_dict[new_key_btnk] = v
                    if args.net_version in ['v1d2', 'v1nt', 'v1d3', 'pure_temporal']:
                        new_key_tmpr = res.group(1) + 'temporal.' + res.group(2)
                    ...
                    if 'transition' in key:
                        new_key_btnk = res.group(1) + 'original.' + res.group(2)
                        state_dict[new_key_btnk] = v
                        if args.net_version in ['v1d2', 'vt', 'v1d3', 'pure_temporal']:
                            new_key_tmpr = res.group(1) + 'temporal.' + res.group(2)
                            state_dict[new_key_tmpr] = v
                        del state_dict[key]
                    else:
...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, split=1, split_pattern=None):
        ...
        if 'pure_temporal' not in args.net_version:
            self.bottleneck = nn.Sequential(OrderedDict([
                ('norm1', BatchNorm3d(num_input_features)),
                ('relu1', nn.ReLU(inplace=True)),
                ('conv1', Conv3d(num_input_features, bn_size *
                                 growth_rate, kernel_size=1, stride=1, bias=False, split=split,
        ...
        self.spatial = nn.Sequential(OrderedDict([
            ...
            ('conv2', Conv3d(bn_size * growth_rate, growth_rate,
                             kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), bias=False, split=1, deact_nasas=_TEMPORAL_NASAS_ONLY))
        ]))
        ...
        if 'v1' in args.net_version:
            ...
            self.temporal = nn.Sequential(OrderedDict([
                ...
                ('conv1', Conv3d(bn_size * growth_rate, bn_size * growth_rate,
                                 kernel_size=(3, 1, 1), stride=1, padding=(1, 0, 0), bias=False, split=1))
            ]))
        ...
        elif 'pure_temporal' in args.net_version:
            self.temporal = nn.Sequential(OrderedDict([
                ('norm1', BatchNorm3d(num_input_features)),
                ('relu1', nn.ReLU(inplace=True)),
                ('conv1', Conv3d(num_input_features, bn_size *
    ...
    def forward(self, x):
        if 'v1' in args.net_version:
            ...
            new_features = self.temporal.forward(x)
            new_features = self.spatial.forward(new_features)
        else:
            new_features = self.bottleneck.forward(x)
            new_features = self.spatial.forward(new_features)
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, split=1, split_pattern=None, temporal_pool_size=1):
        ...
        if args.net_version in ['v1', 'v1d2', 'vt', 'v1d3']:
            self.temporal = nn.Sequential(OrderedDict([
                ...
                ('relu', nn.ReLU(inplace=True)),
                ('conv', Conv3d(num_input_features, num_output_features,
                                kernel_size=(3, 1, 1), stride=1, padding=(1, 0, 0), bias=False, split=split,
                                split_pattern=split_pattern))
            ]))
    ...
    def forward(self, input):
        if args.net_version in ['v1', 'v1d2', 'vt', 'v1d3']:
            ...
            new_features = self.temporal(input)
        else:
            new_features = self.original(input)
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv3d(3, num_init_features, kernel_size=(5, 7, 7), stride=(1, 2, 2) if _TEMPORAL_NODOWNSAMPLE else 2, padding=(2, 3, 3), bias=False)),
            ('norm0', BatchNorm3d(num_init_features)),
            ('relu0', nn.ReLU(inplace=True)),
            ('pool0', nn.MaxPool3d(kernel_size=3, stride=(1, 2, 2) if _TEMPORAL_NODOWNSAMPLE else 2, padding=1)),
        ]))

        # Each denseblock
        num_features = num_init_features
        downsample_pos = [-1] if _TEMPORAL_NODOWNSAMPLE else [0]
